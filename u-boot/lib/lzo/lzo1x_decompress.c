...
#ifdef CONFIG_NT96660 //for FAST BOOT
#include "../../board/novatek/evb660/nvt_type.h"
#include "../../board/novatek/evb660/BinInfo.h"
		    ...
		    unsigned char *dst, size_t *dst_len)
{
	...
	#ifdef CONFIG_NT96660 //for FAST BOOT
	const unsigned char *ori_src = src;
        const volatile unsigned int* load_len = (const volatile unsigned int*)dma_getNonCacheAddr((unsigned int)(&gBinInfo.ld.LdLoadSize));
	#endif
	...
	while (src < send) {
		...
		unsigned long headerlen = 12;
		if(headerlen > (send - src))
		{
			headerlen = (send - src);
		}
		while(src+headerlen>ori_src+*load_len);//wait data
		#endif
		...
		#ifdef CONFIG_NT96660
		while(src+slen>ori_src+*load_len);//wait data
		#endif 
