...
static ulong malloc_addr = CONFIG_SYS_MONITOR_BASE + (CFG_MEM_SIZE<<20) - TOTAL_MALLOC_LEN;//just a temp value
#endif
...
void board_init_f(ulong bootflag)
{
	 ...
	 */
#ifdef CONFIG_NT96660
	addr = CONFIG_SYS_MONITOR_BASE + gd->ram_size;
#else
	...
	*s-- = 0;
	 ...
	 */
#ifdef CONFIG_NT96660
	bd->bi_memstart	= CONFIG_SYS_MONITOR_BASE;	/* start of  DRAM memory */
#else
...
#if CONFIG_NT96660
	relocate_code (addr_sp, id, CONFIG_SYS_MONITOR_BASE);
#else
...
void board_init_r(gd_t *id, ulong dest_addr)
{
	...
	bd = gd->bd;
#ifdef CONFIG_NT96660
	mem_malloc_init(malloc_addr, TOTAL_MALLOC_LEN);
#else
