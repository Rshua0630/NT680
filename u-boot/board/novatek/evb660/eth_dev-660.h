...
#ifndef NVT_ETH_DEV_H
#define NVT_ETH_DEV_H 1
...
#include <miiphy.h>
//#include <malloc.h>
#include <asm/arch-nt96660/nvt_plat.h>
#include "rcw_macro.h"
...
#define DEFAULT_PHY_BASE PHY3		//RTL Phy 
...
#define MDC_CLOCK_DIV 			GmiiCsrClk1 // for ahb 125MHz
								//GmiiCsrClk3 // for ahb 48MHz
...
//#define RECEIVE_DESC_SIZE   8 //256	//Rx Descriptors needed in the Descriptor pool/queue
...
typedef struct ethDeviceStruct      
{
  ...
  DmaDesc *TxDescBase;		/* start address of TX descriptors ring or chain, this is used by the driver  */
  DmaDesc *RxDescBase;		/* start address of RX descriptors ring or chain, this is used by the driver  */

  u32 BusyTxDescNum;		/* Number of Tx Descriptors owned by DMA at any given time*/  
  u32 BusyRxDescNum;		/* Number of Rx Descriptors owned by DMA at any given time*/  
  ...
  u32 TxBusyIndx;			/* index of the tx descriptor owned by DMA, is obtained by eth_get_tx_qptr()                */
  u32 TxNextIndx;			/* index of the tx descriptor next available with driver, given to DMA by eth_set_tx_qptr() */
  u32 RxBusyIndx;			/* index of the rx descriptor owned by DMA, obtained by eth_get_rx_qptr()                   */
  u32 RxNextIndx;			/* index of the rx descriptor next available with driver, given to DMA by eth_set_rx_qptr() */

  DmaDesc *TxBusyDesc;		/* Tx Descriptor address corresponding to the index TxBusyIndx */
  DmaDesc *TxNextDesc;		/* Tx Descriptor address corresponding to the index TxNextIndx */
  DmaDesc *RxBusyDesc;		/* Rx Descriptor address corresponding to the index TxBusyIndx */
  DmaDesc *RxNextDesc;		/* Rx Descriptor address corresponding to the index RxNextIndx */
...
enum MiiRegisters
{
  ...
  PHY_PAGE_REG		    = 0x001f,    	/* Howard Chang: RTL page register          */
  PHY_TEST_REG		    = 0x0019,	        /* Howard Chang: RTL8201E Test register     */
  PHY_LBREM_REG 	    = 0x0011,	        /* Howard Chang: RTL8201E LBREM register    */
  PHY_RXER_REG 		    = 0x0012,	        /* Howard Chang: RTL8201E RX ERROR register */
};
...
{ 
  ...
  DmaDescriptorSkip12     = 0x00000030,
 ...
 **********************************************************/
 ...
 enum ethDmaIntEnum  /* Intrerrupt types */
{
  ethDmaRxNormal   = 0x01,   /* normal receiver interrupt */
  ethDmaRxAbnormal = 0x02,   /* abnormal receiver interrupt */
  ethDmaRxStopped  = 0x04,   /* receiver stopped */
  ethDmaTxNormal   = 0x08,   /* normal transmitter interrupt */
  ethDmaTxAbnormal = 0x10,   /* abnormal transmitter interrupt */
  ethDmaTxStopped  = 0x20,   /* transmitter stopped */
  ethDmaError      = 0x80,   /* Dma engine error */
};
...
// Howard Chang: RTL phy functions
s32 synopGMAC_get_phy_test_reg(mac_device * gmacdev);
void synopGMAC_get_phy_LBREM_reg(mac_device * gmacdev);
s32 synopGMAC_RTL8201F_set_phy_TXC_direction(mac_device * gmacdev, u8 input);
s32 synopGMAC_RTL8201F_set_phy_adjust_RMII_RX_timing(mac_device * gmacdev, u8 timing);
#endif /* End of file */
