...
int nvt_emmc_image_help(struct _nvt_emmc_image_info *this, NVT_EMMC_IMG_HELP_TYPE help_type)
{
	...
	switch(help_type) {
		...
		case IMG_HELP_WRITE:
			...
			printf("Usage: ewrite %s [%s] - %s\n",this->image_name, this->default_name, tmp_str);
			break;
...
int do_nvt_emmc_check(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if(argc == 2)
	{
		...
		sprintf(cmd,"fatload usb 0 0x%x %s",CONFIG_SYS_FWUPDATE_BUF, img_name);
...
#ifdef CONFIG_CMD_NVTUPDATE

#ifdef CONFIG_SYS_HUSH_PARSER
#define FLAG_PARSE_SEMICOLON (1 << 1)               /* symbol ';' is special for parser */
#endif

int do_nvt_update(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong addr;
	int rcode = 0, i, j;
    char *cmd, *buf, *delim;
    ...
    if (argc < 2) {
        sprintf(cmd2, "fatload usb 0 0x%x %s", addr, CONFIG_AUTO_SCRIPT);
    } else {
        if (strcmp(argv[1], "help") == 0)
        {
            return CMD_RET_USAGE;
        ...
        sprintf(cmd2, "fatload usb 0 0x%x %s", addr, argv[1]);
    }

    if ((rcode = run_command(cmd2, 0))) {
        printf("load script file failed!\n");
        return rcode;
    }

    printf("USB AUTO UPDATE!!\n");
	printf ("## Executing auto script at %08lx\n", addr);
    buf = addr;

    for (i = 0; i < CONFIG_AUTO_SCRIPT_SIZE; i++) {
        if (buf[i] == '%')
            break;
    }

    if (i == CONFIG_AUTO_SCRIPT_SIZE) {
