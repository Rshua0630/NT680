...
int do_n62001_boot_cmd(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if(getenv("p3") != NULL) {
        sprintf(buf,"%s ","root=/dev/mmcblk0p8 rw ");
		setenv("ramdisk_loadaddr", NULL);
	}else
#endif
		if(getenv("usb") != NULL){
...
int do_n33002_boot_cmd(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	char cmd[512] = {0};
	char buf[255] = {0};
	int ret = 0,len;
	...
	if (ret == INVAILD_USER_PART_ID) {
		printf("%s get fs0 part id fail !\n",__func__);
		goto loop;
	}
    sprintf(buf,"root=/dev/mmcblk0p%d rw ", ret);
    strcat(cmd,buf);
	setenv("ramdisk_loadaddr", NULL);
...
U_BOOT_CMD(
		n33002_boot,	2,	0,	do_n33002_boot_cmd,
		"",""
	  );
#endif //CONFIG_N33002
