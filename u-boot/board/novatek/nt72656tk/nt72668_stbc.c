...
FAC_STBC_DATA  __attribute__((__aligned__(64))) gstStbcData; 
...
int nt72668_stbc_read_data(FAC_STBC_DATA* pData)
{
	int ret;
	char cmd[64];
...
int nt72668_stbc_boot_check(void)
{
	...
	/* Get Power on mode */
	nt72668_stbc_read_data(&gstStbcData);

	
#ifdef KEYPAD_LOCK_M
	if(gstStbcData.KeypadLockFlag == KEYPAD_UNLOCK)
	{
		//send cmd to stbc
		nt72668_stbc_unlock_keypad();
	}
	else if(gstStbcData.KeypadLockFlag == KEYPAD_LOCK)
	{
		//send cmd to stbc
...
#ifdef AUTO_CEC_M
if(gstStbcData.CecAutoPowerOnMode == AUTO_POWER_ON)
{
	//send cmd to stbc
	nt72668_stbc_cec_auto_on();
}
else if(gstStbcData.CecAutoPowerOnMode == AUTO_POWER_OFF)
{
	//send cmd to stbc
...
else
{
	...
	{
		switch(gstStbcData.PowerOnMode) {
			...
			default:
				if(gstStbcData.PowerStatus == STBC_PWR_STANDBY)
				{						
