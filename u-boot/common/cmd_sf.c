...
static int do_spi_flash_read_status(int argc, char * const argv[])
{
	u8 status;
	int ret;
	...
	if(ret < 0)
		printf("Failed to read spi flash status!\n");
	else
		printf("status 0x%x\n", status);
			...
			char * const argv[])
{
	...
	if (strcmp(cmd, "read") == 0 || strcmp(cmd, "write") == 0 ||
		...
		ret = do_spi_flash_read_status(argc, argv);
	else
		ret = -1;
...
U_BOOT_CMD(
	...
	"				  at `addr' to flash at `offset'\n"
	"sf status			- read status register\n"
	SF_TEST_HELP
);
