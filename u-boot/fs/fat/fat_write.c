...
/*
 * Remove the full long filename starting at 'retdent' (which is really
 * a slot). If successful also copy the real directory entry into 'retdent'
 * If additional adjacent cluster for directory entries is read into memory,
 * then 'get_contents_vfatname_block' is copied into 'get_dentfromdir_block' and
 * the location of the real directory entry is returned by 'retdent'
 * Return 0 on success, -1 otherwise.
 */
static int
remove_long_file_name(fsdata *mydata, int curclust, __u8 *cluster,
	      dir_entry **retdent, char *l_name, const char *filename)
{
	dir_entry *realdent;
	dir_slot *slotptr = (dir_slot *)(*retdent);
	dir_slot *slotptr2 = NULL;
	...
	if (0 != strcmp(l_name, filename))
		return -1;

	//remove the long name part
	counter = (slotptr->id & ~LAST_LONG_ENTRY_MASK) & 0xff;

	delptr = slotptr;
	while ((__u8 *)delptr < buflimit) {
		if (counter == 0)
			break;
		delptr->id = DELETED_FLAG;
		delptr++;
		counter--;
	}

	if (slotptr2) {
		//the long file name crosses clusters
		delptr = slotptr2;
		while (counter > 0) {
			delptr->id = DELETED_FLAG;
			delptr++;
	...
	//remove the short name part
	delptr->id = DELETED_FLAG;

	//if the 2nd cluster is modified, write back first
	if (slotptr2) {
		if (0 != set_cluster(mydata, curclust,
			    get_contents_vfatname_block,
			    mydata->clust_size * mydata->sect_size)) {
			printf("Error: writing directory entry slotptr2\n");
			return -1;
		}
	}
...
/*
 * Remove a directory entry based on filename or start cluster number
 * Note: Only the entry is removed. The allocated cluster is not released.
 * Return 0 on success, -1 otherwise.
 */
static int remove_directory_entry(fsdata *mydata, int startsect,
	char *filename, __u32 *start_cluster)
{
	__u32 curclust = (startsect - mydata->data_begin) / mydata->clust_size;
		...
		for (i = 0; i < DIRENTSPERCLUST; i++) {
			...
			if ((dentptr->attr & ATTR_VOLUME)) {
				if (vfat_enabled &&
				    (dentptr->attr & ATTR_VFAT) &&
				    (dentptr->name[0] & LAST_LONG_ENTRY_MASK)) {
					long_match = (0 == remove_long_file_name(mydata, curclust,
									get_dentfromdir_block,
									&dentptr, l_name, filename));
				} else {
					/* Volume label or VFAT entry */
					dentptr++;
					if (is_next_clust(mydata, dentptr))
			...
			if (!long_match) {
				...
				dentptr->name[0] = DELETED_FLAG; //delete short name entry
			}
...
static int do_fat_rename(const char *oldname, const char *newname)
{
	dir_entry *retdent;
	dir_entry olddent, newdent;
	__u32 startsect;
	boot_sector bs;
	volume_info volinfo;
	fsdata datablock;
	...
	if (mydata->fatsize == 32) {
		mydata->data_begin = mydata->rootdir_sect -
					(mydata->clust_size * 2); //intentionally minus 2 cluster size for later use
	} else {
		int rootdir_size;
	...
	//find oldname and copy its short entry
	name_len = strlen(oldname);
	if (name_len >= VFAT_MAXLEN_BYTES)
		name_len = VFAT_MAXLEN_BYTES - 1;

	memcpy(l_filename, oldname, name_len);
	l_filename[name_len] = 0; /* terminate the string */
	downcase(l_filename);

	startsect = mydata->rootdir_sect;
	if (NULL == find_directory_entry(mydata, startsect,
				l_filename, &olddent, 0)) {
		printf("Error: %s not found\n", l_filename);
		goto exit_rename;
	}

	//find newname and make sure it doesn't exist
	name_len = strlen(newname);
	if (name_len >= VFAT_MAXLEN_BYTES)
		name_len = VFAT_MAXLEN_BYTES - 1;

	memcpy(l_filename, newname, name_len);
	l_filename[name_len] = 0; /* terminate the string */
	downcase(l_filename);

	startsect = mydata->rootdir_sect;
	retdent = find_directory_entry(mydata, startsect,
				l_filename, &newdent, 0);
	if (NULL != retdent) {
		printf("Error: %s should not exist\n", l_filename);
		goto exit_rename;
	}

	//create newname with oldname attrib
	set_name(empty_dentptr, newname);
	fill_dir_slot(mydata, &empty_dentptr, newname);

	fill_dentry(mydata, empty_dentptr, newname,
		START(&olddent), FAT2CPU32(olddent.size), olddent.attr);
	...
	startsect = mydata->rootdir_sect;
	if (-1 == remove_directory_entry(mydata, startsect,
				l_filename, NULL)) {
		printf("Error: %s not found\n", l_filename);
		goto exit_rename;
	}
...
static int do_fat_rm(const char *filename)
{
	...
	/* Flush fat buffer */
	/* clear_fatent() already flush the buffer
	ret = flush_fat_buffer(mydata);
	if (ret) {
		printf("Error: flush fat buffer\n");
		goto exit_rm;
...
int file_fat_rename(const char *oldname, const char *newname)
{
	...
	return do_fat_rename(oldname, newname);
}
