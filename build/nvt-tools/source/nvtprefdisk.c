...
#define UNUSED_PARAM
#define bb_error_msg(fmtstr, args...) printf("\033[0;31mERR:%s() \033[0m" fmtstr, __func__, ##args)
#define bb_perror_msg(fmtstr, args...) printf("\033[0;31mERR:%s() \033[0m" fmtstr, __func__, ##args)
#define bb_perror_msg_and_die(fmtstr, args...) do {printf("\033[0;31mERR:%s() \033[0m" fmtstr, __func__, ##args); exit -1;} while(0)
#define bb_error_msg_and_die(str) do {printf("\033[0;31mERR:%s() %s \033[0m", __func__, str); exit -1;} while(0)
#define bb_simple_perror_msg(args...) printf("\033[0;31mERR:%s() %s \033[0m", __func__, ##args)
...
#if NVT_EXTRACT
	sz = read_line_input(prompt, line_buffer, sizeof(line_buffer));
#else
...
static void
createfile(sector_t secno, const void *buf, size_t count)
{
	char szpath[32];
	int fd;

	snprintf(szpath, sizeof(szpath), "MBR_%u.bin", secno);

	fd = open(szpath, O_RDWR|O_CREAT|O_TRUNC, 0666);
	if (fd == -1) {
...
{
	printf("write_sector %u\r\n", secno);
	createfile(secno, buf, sector_size);
...
add_partition(int n, int sys)
{
	...
	if (n > 4) {                    /* NOT for fifth partition */
		...
		#if NVT_EXTRACT
		pe->offset_from_dev_start = last[n-1] + 1;
		#else
	...
	if (LABEL_IS_DOS) {
		...
		for (i = 3; i < g_partitions; i++) {
			...
			if (pe->changed) {
				...
				write_sector(j++, pe->sectorbuffer);
			}
		}
	}
	else if (LABEL_IS_SGI) {
...
#if NVT_EXTRACT
	if (argc < 3) {
		printf("Please enter total sectors\r\n");
		return -1;
	}

	total_number_of_sectors = strtoul(argv[1], NULL, 10);
#endif
