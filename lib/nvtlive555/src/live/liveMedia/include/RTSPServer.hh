...
public:
  ...
  virtual ServerMediaSession* lookupServerMediaSession(char const* streamName,int isForceClean=0, char const* url = NULL); //NVT_MODIFIED, in order to clean sms at each rtsp-desc command
...
public: // should be protected, but some old compilers complain otherwise
  ...
  public:
    // NVT_MODIFIED, fix sms leaking (detail)
	// our strategy is, one client has its own sms.
	// it differs from original liv555's. original use shared sms if the url is the same
	// so we use sequence number to be a unique session name to avoid look up getting sms that used before
  ...
  protected:
	...
	void **m_ppRTSPClientConnection;
	void setNotifySuicide(void **ppRTSPClientConnection);
    //NVT_MODIFIED, fix sms leaking
	unsigned int m_connected_sn;
  };
  ...
  protected:
	...
	RTSPClientConnection* fOurClientConnection; //NVT_MODIFIED, fix timeout without release accepted socket.
    struct streamState {
      ServerMediaSubsession* subsession;
      void* streamToken;
    } * fStreamStates;
...
protected:
  ...
  virtual RTSPClientConnection*
  //NVT_MODIFIED, fix sms leaking
  createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, unsigned int connected_sn);

  // If you subclass "RTSPClientSession", then you must also redefine this virtual function in order
  // to create new objects of your subclass:
  virtual RTSPClientSession*
	  createNewClientSession(u_int32_t sessionId, RTSPClientConnection* ourClientConnection); //NVT_MODIFIED, fix timeout without release accepted socket.
...
//NVT_MODIFIED, fix sms leaking
public:
  unsigned int m_current_connected_sn;
};
