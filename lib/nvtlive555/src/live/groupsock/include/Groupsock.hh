...
public:
  ...
  Boolean write(netAddressBits address, Port port, u_int8_t ttl,
		unsigned char* buffer, unsigned bufferSize, int idx_socket=0, int* pByteSent=NULL); //NVT_MODIFIED, Multi-UDP Source, //NVT_MODIFIED, WIFI-TEAM
...
public:
  ...
  //NVT_MODIFIED, get dest ip address //wifiteam
  struct in_addr const& destAddress() const {
      in_addr zero_addr = { 0 };
      return (fDests) ? fDests->fGroupEId.groupAddress() : zero_addr;
  }
  ...
  Boolean output(UsageEnvironment& env, u_int8_t ttl,
		 ...
		 DirectedNetInterface* interfaceNotToFwdBackTo = NULL,int idx_socket = 0, int* pByteSent=NULL); //NVT_MODIFIED, Multi-UDP Source //NVT_MODIFIED, WIFI-TEAM
