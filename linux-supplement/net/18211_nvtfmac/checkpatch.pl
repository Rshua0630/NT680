...
# check if we are using tab
		if ($line =~ /\t/)  {
			my $herevet = "$here\n" . cat_vet($rawline) . "\n";
			WARN("TAB",
				 "No tab, use spaces please\n" . $herevet);
		}

# check if we are using wrong naming
		if ($line =~ /brcm/)  {
			my $herevet = "$here\n" . cat_vet($rawline) . "\n";
			WARN("TAB",
				 "Fix naming here please\n" . $herevet);
		}
...
# check if there is space follow else
		if ($line =~ /else\{/)  {
			my $herevet = "$here\n" . cat_vet($rawline) . "\n";
			WARN("TAB",
				 "space reuqired between else and {\n" . $herevet);
		}
