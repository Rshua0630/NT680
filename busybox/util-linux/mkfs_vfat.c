...
int mkfs_vfat_main(int argc UNUSED_PARAM, char **argv)
{
	...
	uint8_t heads;
	//#NT#2018/01/16#Nestor Yang -begin
	//#NT# Format with alignment
	uint16_t uiPBR_ResvSecCount;
	//#NT#2018/01/16#Nestor Yang -end
	enum {
 ...
 set_cluster_size:
			...
			if (volume_size_bytes >= 260*1024*1024) {
				...
				if (sizeof(off_t) > 4) {
					...
					//Enlarge the cluster size to improve rec performance
					#if 0
					if (t >= 8/4)
						sect_per_clust = 16;
					if (t >= 16/4)
...
//#NT#2018/01/17#Nestor Yang -begin
//#NT# Format with alignment
//if the disk size is big enouth (>=512MB), format with alignment
if ((off_t)(volume_size_sect) >= (1024*1024)) {
	// 1. sector per cluster (SC): sect_per_clust
	// 2. sector size (SS): bytes_per_sect
	// 3. total sectors (TS): volume_size_sect
	// 4. FAT bits: uiFatBits(32)
	// 5. Sector per FAT: sect_per_fat = ceil( volume_size_sect/sect_per_clust*uiFatBits / (bytes_per_sect * 8) )
	// 6. Max cluster number: total_clust
	{
		#if 0
		#define DBG_MSG(fmtstr, args...) fprintf(stderr, fmtstr, ##args)
		#else
		...
		unsigned int uiSysAreaSize;
		unsigned int uiFatBits = 32;
		unsigned int uiBU = 8192;
		unsigned int uiMBR_RelativeSec;
		unsigned int uiSectorPerFatPrime;
		unsigned int i;

		sect_per_fat = ALIGN_CEIL(volume_size_sect / sect_per_clust * uiFatBits, (bytes_per_sect * 8)) / (bytes_per_sect * 8);
		DBG_MSG("[fmt]SF = %d\r\n",sect_per_fat);

		// 6. number of sectors in MBR(NOM): NOM = BU
		uiMBR_RelativeSec = 0; //since 1st sector is starting from partition1, uiMBR_RelativeSec is not needed
		DBG_MSG("[fmt]NOM = %d\r\n", uiMBR_RelativeSec);

		do {
		// 7. Reserved Sector Count (RSC): RSC = BU * n - 2 * SF
			for (i=1; uiBU*i <= 2*sect_per_fat; i++);
			uiPBR_ResvSecCount = uiBU*i - 2*sect_per_fat;
			DBG_MSG("[fmt]RSC = %d\r\n", uiPBR_ResvSecCount);

		// 8. If RSC is smaller than 9, RSC is added BU.
			if (uiPBR_ResvSecCount < 9)
			{
				uiPBR_ResvSecCount += uiBU;
				DBG_MSG("[fmt]RSC(2) = %d\r\n", uiPBR_ResvSecCount);
			}

		// 9. number of sectors in the system area (SSA):
		//		SSA = RSC + 2*SF
			uiSysAreaSize = uiPBR_ResvSecCount + 2*sect_per_fat;
			DBG_MSG("[fmt]SSA = %d\r\n",uiSysAreaSize);
		...
		//		MAX = ip ( (TS-NOM-SSA) / SC ) + 1
				total_clust = (volume_size_sect - uiMBR_RelativeSec - uiSysAreaSize) / sect_per_clust + 1;
				DBG_MSG("[fmt]MAX = %d\r\n", total_clust);

		// 11. recalculate SF
		//		SF' = ceil { [2 + (MAX - 1)] * FAT bits / (SS * 8) }
				uiSectorPerFatPrime = ALIGN_CEIL((2 + (total_clust - 1)) * uiFatBits, (bytes_per_sect * 8)) / (bytes_per_sect * 8);
				DBG_MSG("[fmt]SF' = %d\r\n", uiSectorPerFatPrime);

		// 12. If SF' is greater than SF, SSA and RSC are added BU.
				if (uiSectorPerFatPrime > sect_per_fat)
				{
					uiPBR_ResvSecCount += uiBU;
					uiSysAreaSize += uiBU;
					DBG_MSG("[fmt]new SSA = %d\r\n",uiSysAreaSize);
					DBG_MSG("[fmt]new RSC = %d\r\n",uiPBR_ResvSecCount);
				}
			} while (uiSectorPerFatPrime > sect_per_fat);

		// 13. If SF' != SF, 'SF-1' is used as new SF
			if (uiSectorPerFatPrime != sect_per_fat)
			{
				sect_per_fat--;
				DBG_MSG("[fmt]new SF = %d\r\n", sect_per_fat);
			}
			else
			{
				break;
	...
	uiPBR_ResvSecCount = reserved_sect;
	sect_per_fat = 1;
	while (1) {
		while (1) {
			int spf_adj;
...
//#NT#2018/01/17#Nestor Yang -end
 found_total_clust:
	...
	{ // boot and fsinfo sectors, and their copies
		...
		STORE_LE(boot_blk->reserved_sect, (uint16_t)uiPBR_ResvSecCount);
		//#NT#2018/01/16#Nestor Yang -end
	...
	{ // file allocation tables
		...
		//#NT#2018/01/16#Nestor Yang -begin
		//#NT# Format with alignment
		xlseek(dev, uiPBR_ResvSecCount*bytes_per_sect, SEEK_SET);
		//#NT#2018/01/16#Nestor Yang -end
