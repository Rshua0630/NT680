...
#define VDODISP_API_VERSION 0x17040512U
...
typedef enum _VDODISP_CMD_IDX {
	...
	VDODISP_CMD_IDX_SET_FLIP,        ///< flip a frame (In:IMG_BUF*, Out: NULL)
	VDODISP_CMD_IDX_DUMP_FLIP,       ///< dump current displayed frame to A:\\disp_xxx_xxx.raw (In:NULL, Out: NULL)
	VDODISP_CMD_IDX_RELEASE,         ///< to release locked frame by using temp buffer to freeze a last frame or by setting ide disable (In:IMG_BUF*, Out: NULL)
...
typedef struct _VDODISP_DISP_DESC {
	...
	ISIZE                   device_size; ///< device size to meet HSync and VSync for calculating best buffer size.(the value normally are 960x480,960x240,640x480,640x240 for LCD)
...
typedef void (*VDODISP_FP_RELEASE_CB)(UINT32 device_id, IMG_BUF *img_buf); ///< notify this frame is released.
...
/**
     ...
     @endcode
*/
extern void vdodisp_install_id(void) _SECTION(".kercfg_text");
...
*/
extern VDODISP_ER vdodisp_init(const VDODISP_INIT *p_Init);
...
*/
extern VDODISP_ER vdodisp_open(VOID);
...
/**
    ...
    @return
        ...
        - @b VDODISP_ER_STA_TASK: failed to start task(sta_tsk failed).
*/
extern VDODISP_ER vdodisp_close(VOID);
...
*/
extern VDODISP_ER vdodisp_suspend(VOID);
...
/**
    ...
    @return
        ...
        - @b VDODISP_ER_STATE: may vdodisp_open does not be opened yet.
*/
extern VDODISP_ER vdodisp_resume(VOID);
...
/**
    ...
    @return
        - @b VDODISP_ER_OK: suspend success.
*/
extern VOID vdodisp_dump_info(void (*dump)(char *fmt, ...));
#endif
