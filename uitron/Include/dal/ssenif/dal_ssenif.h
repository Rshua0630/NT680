...
typedef enum {
	...
	DAL_SSENIFCSI_CFGID_IMGID_TO_SIE3,     	///< [HDR] Select IMAGE frame ID sent to SIE3. Valid for CSI only.
	...
	DAL_SSENIFCSI_CFGID_MANUAL_FORMAT,		///<[OPTIONAL] Select the depacking method for Manual1 packet.
											///<           Please use CSI_MANDEPACK as input parameter.
...
typedef enum {
	...
	DAL_SSENIFLVDS_CFGID_PIXEL_INORDER,		///<[OPTIONAL] Select Pixel Data bit order MSb or LSb first. Use LVDS_DATAIN_BIT_ORDER as input.
...
typedef enum {
	...
	DAL_SSENIF_ID_SLVSEC,   ///< DAL of Serial Sensor Interface: SLVS-EC module.
