...
/**
    ...
    (2) aud_setExtCodec(&gExtCodecFunc);
...
/**
    ...
    For example, you should invoke aud_switchCodec(AUDIO_CODECSEL_HDMI) if HDMI plug is detected.
    After HDMI cable is removed, you may invoke aud_switchCodec(AUDIO_CODECSEL_DEFAULT) to switch back to embedded/external audio codec.
...
/**
    ...
    parameter "vol" range of API aud_setVolume() is from AUDIO_VOL_MUTE ~ AUDIO_VOL_7.
    When the parameter "Audio_VolLevel" is set to AUDIO_VOL_LEVEL64, the valid parmeter
    "vol" range of API aud_setVolume() is from AUDIO_VOL_MUTE ~ AUDIO_VOL_63.
