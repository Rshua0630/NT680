...
typedef struct ROTATION_IMG {
	ROTATION_IMG_ID imgID;          ///< Image identifier
	...
	struct ROTATION_IMG *pNext;         ///< Link to next image. (Fill NULL if this is last item)
...
typedef struct {
	...
	PROTATION_IMG pImageDescript;   ///< Descriptions of images for command
...
// Functions exported from rotation driver
extern ER       rotation_open(ROTATION_ID id);
extern BOOL     rotation_isOpened(ROTATION_ID id);
extern ER       rotation_close(ROTATION_ID id);

extern ER       rotation_setConfig(ROTATION_ID rotationID, ROTATION_CONFIG_ID configID, UINT32 configContext);
extern ER       rotation_getConfig(ROTATION_ID rotationID, ROTATION_CONFIG_ID configID, UINT32 *pConfigContext);

extern ER       rotation_request(ROTATION_ID id, PROTATION_REQUEST pRequest);
