...
extern cyg_uint32 kreg_mpf(cyg_uint32 *puiSize);
extern cyg_bool   kchk_mpf(void);
extern cyg_bool   kchk_stk(void);
extern ER         kreg_sem(ID semid, cyg_int32 MaxCnt);
extern cyg_int32  kget_sem(ID semid);

extern cyg_uint32 memsta_alloc_stksize;

#define CYGNUM_ADDITIONAL_STACK_SIZE    (CYGNUM_HAL_STACK_SIZE_MINIMUM + 2048)
...
cre_tsk ( ID tskid, T_CTSK *pk_ctsk )
{
    ...
    if(kchk_stk())
    {
        ...
        // (1)According to ecos reference manual, the CYGNUM_HAL_STACK_SIZE_MINIMUM is required minimum stack size
        // to make context switch and interrupr working normally. So each application task stack needs to
        // add additional size of CYGNUM_HAL_STACK_SIZE_MINIMUM.
        // (2)Besides,the additional 2K bytes is because the standard lib c such as printf/sprintf/...
        // Needs more stack sizes than original uitron. We add 2K bytes of stack for each uitron tasks
        // To make the original application working normally.
        pk_ctsk->stksz += CYGNUM_ADDITIONAL_STACK_SIZE;

        if ( 0 == (state & Cyg_Thread::EXITED) )
        {
            ret = E_OBJ; // how did it get to be running?
        }
        else if(pk_ctsk->stksz <= CYGNUM_HAL_STACK_SIZE_MINIMUM)
        {
            ret = E_NOMEM;
        }
        else
        {
            ...
            // malloced address is already 32bytes aligned. Need not handle alignment anymore.
            block = (cyg_int32 *)malloc(pk_ctsk->stksz);

            // For Heap usage statistics data dump information
            memsta_alloc_stksize+=(pk_ctsk->stksz);

            if(block == NULL)
            {
                diag_printf("cre_tsk(): malloc fail!\r\n");
                ret = E_NOMEM;
                Cyg_Scheduler::unlock();
                return ret;
            }
            ...
            //diag_printf("Start NVT Task %s(): Stack Addr=0x%08X Size=0x%08X\r\n",p->get_name(),(INT)block,pk_ctsk->stksz);
        }
    ...
    else
    {
        ...
        //diag_printf("Start Task %s(): Stack Addr=0x%08X Size=0x%08X\r\n",p->get_name(),p->get_stack_base(),p->get_stack_size());
    }
...
#endif
            // Novatek ivot interrupt service tasks use priority 2 and 3
            // Move killed tasks priority from 0 to 3,
            // else killed tasks may block IST
            p->set_priority( (cyg_priority) 3 );
...
ref_tsk ( T_RTSK *pk_rtsk, ID tskid )
{
    ...
    pk_rtsk->stk     = (VP)p->get_stack_base();
    pk_rtsk->stksz   = p->get_stack_size();
    pk_rtsk->stkused = p->measure_stack_usage();
...
cre_sem ( ID semid, T_CSEM *pk_csem )
{
    ...
    // store the max semaphore count for each semid
    kreg_sem(semid, pk_csem->maxsem);
...
sig_sem( ID semid )
{
    ...
    // NVT: Clamp the counting Number if reached the max count
    if(p->peek() < kget_sem(semid))
        p->post();

    //diag_printf("ID(%d) NUM= %d\r\n",semid,p->peek());
    return E_OK;
}
...
cre_mpf ( ID mpfid, T_CMPF *pk_cmpf )
{
    ...
    else
    {
        ...
        poolsize = (cyg_uint32)(pk_cmpf->blfsz * (pk_cmpf->mpfcnt));
        pAddr = (cyg_uint8 *)kreg_mpf((cyg_uint32 *)&poolsize);
        if(pAddr == NULL)
            return E_NOMEM;

        //diag_printf("=====ID %d=====\r\n",mpfid);
        //diag_printf("Base Address = 0x%08X\r\n",pAddr);
        //diag_printf("Block Size   = 0x%08X\r\n",pk_cmpf->blfsz);

        if ( TA_TFIFO != pk_cmpf->mpfatr )
            ret = E_RSATR;
        else
            CYG_UITRON_PTRS( MEMPOOLFIXED )[ mpfid - 1 ] =
                new( p )
                Cyg_Mempool_Fixed( pAddr, poolsize, (CYG_ADDRWORD)(poolsize-32) );
    }
