...
inline void Cyg_HardwareThread::check_stack(void)
{
    ...
    for ( i = 0;
        ...
        if ((sig ^ (i * 0x01010101)) != base[i]) {
            char *reason = (char *)"Stack base corrupt";
            Cyg_Thread* thisThread = (Cyg_Thread *)this;
            diag_printf("Thread \"%s\":: %s - i: %d\n", thisThread->get_name(), reason, i);
            diag_dump_buf(base, CYGNUM_KERNEL_THREADS_STACK_CHECK_DATA_SIZE);
            CYG_FAIL(reason);
        }
        if ((sig ^ (i * 0x10101010)) != top[i]) {
            char *reason = (char *)"Stack top corrupt";
            Cyg_Thread* thisThread = (Cyg_Thread *)this;
            diag_printf("Thread \"%s\":: %s - i: %d\n", thisThread->get_name(), reason, i);
    ...
    if (stack_limit != stack_base) {
        ...
        for ( i = 0;
            ...
            if ((sig ^ (i * 0x01010101)) != p[i]) {
                char *reason = (char *)"Gap between stack limit and base corrupt";
                diag_printf("%s - i: %d\n", reason, i);
