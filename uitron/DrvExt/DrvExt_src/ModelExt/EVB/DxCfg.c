...
extern DX_OBJECT gDevUSB0;
extern DX_OBJECT gDevUSB1;
...
DX_HANDLE Dx_GetObject(UINT32 DxClassType)  // Query device object
{
	...
	//#NT#2018/01/12#Ben Wang -begin
	//#NT#Support dual USB device detection
		if ((DxClassType & DX_TYPE_MASK) == DX_TYPE_USB0) {
			hDevice = (UINT32)(&gDevUSB0);
		} else if ((DxClassType & DX_TYPE_MASK) == DX_TYPE_USB1) {
			hDevice = (UINT32)(&gDevUSB1);
		}
	//#NT#2018/01/12#Ben Wang -end
		break;
