...
#if (defined(_CMOS_IMX291P_))
static ER sensor_vx1_tx_gpio(SENSOR_ID id, VX1_TXMODULE tx_module)
{
	ER rt;
	BOOL bLevel = TRUE;
	bLevel = TRUE;
	rt = sensor_if_get_vx1_ctrl_obj()->cfgTransmitterGPIO(id, tx_module, VX1_TXGPIO_3, VX1_GPIODIR_OUTPUT, &bLevel);
	Delay_DelayMs(10);
	return rt;
}

static SENSOR_INIT_OBJ sensor_get_obj_imx291p(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_Vx1;
	init_obj.cmd_info.info.i2c.i2c = SENSOR_I2C_VX1;
	init_obj.cmd_info.info.i2c.w_addr_sel = SEN_I2C_W_ADDR_DFT;
	init_obj.sen_map_if = SENSOR_MAP_IF_DEFAULT;

    init_obj.cmd_info.vx1.tx_module = VX1_TXMODULE_THCV231;
    init_obj.cmd_info.vx1.lane_num = SEN_VX1_1LANE;
    init_obj.cmd_info.vx1.data_mux = SEN_VX1_DATAMUX_4BYTE_RAW;
    init_obj.cmd_info.vx1.fp_tx_gpio = sensor_vx1_tx_gpio;
    init_obj.cmd_info.vx1.cko_pin = TRUE;

	return init_obj;
}
static void sen_power_ctrl_imx291p(SENSOR_ID id, POW_CTRL_FLAG flag)
{
    UINT32 reslut = 0;
	...
	case POW_CTRL_TURN_ON:
        ...
        //vx1_checkSublinkReady();
        reslut = vx1_checkSublinkReady();
	    DBG_ERR("vx1_checkSublinkReady = %d \r\n",reslut);
		// set clock information
		//pll_setClkEn(PLL_CLK_SIEMCLK, ENABLE);
		break;
...
#if (defined(_CMOS_IMX291P_))
	{"CMOS_IMX291P", sensor_get_drv_tab_imx291p, sensor_get_obj_imx291p, sen_power_ctrl_imx291p, NULL, NULL},
#endif
