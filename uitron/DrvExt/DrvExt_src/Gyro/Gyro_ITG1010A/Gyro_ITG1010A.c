...
ER gyro_open(GYRO_SENSOR_ID gID, GYRO_OPENOBJ *openObj)
{
	...
	// initializa gyro
	{
		...
		spi_writeReadSingle(gyroObj[gID].spiID, 0x00, &recv2);
...
static void gyro_syncEnd(GYRO_INNEROBJ *pGyroObj)
{
	...
	if (*gyroBufInIdx == 0) {
		*gyroQueueCount = pGyroObj->frDataNum - pGyroObj->frTriggerIdx;
	} else {
		*gyroQueueCount = pGyroObj->frDataNum;
	}

	for (i = 0; i < *gyroQueueCount; i++) {
		Status = spi_getGyroData(pGyroObj->spiID, &gyroData);
		if (Status != E_OK) {
