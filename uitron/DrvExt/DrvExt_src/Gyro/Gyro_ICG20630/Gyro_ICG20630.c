/*
    Gyro Driver InvenSense ICG20630.

    @file       GyroDrv_ICG20630.c
    @ingroup
    @note       Nothing.
...
static ER gyro_writeRegOneByte(GYRO_SENSOR_ID gID, UINT32 uiAddr, UINT32 uiData);
...
ER gyro_close(GYRO_SENSOR_ID gID)
{
	...
	gyro_writeRegOneByte(gID, REG_PWR_MGMT_2, 0x07);
...
{
	GYRO_SENSOR_ID gID = 0;
	// 111 !V Gyroscope (all axes) disabled
	{
		spi_open(gyroObj[gID].spiID);
		spi_setTransferLen(gyroObj[gID].spiID, SPI_TRANSFER_LEN_1BYTE);
		spi_setCSActive(gyroObj[gID].spiID, TRUE);
		spi_writeSingle(gyroObj[gID].spiID, REG_WRITE | (0x07 & 0x7f));
		spi_writeSingle(gyroObj[gID].spiID, 0x07);
		spi_setCSActive(gyroObj[gID].spiID, FALSE);
		spi_close(gyroObj[gID].spiID);
	}

	{
		// R/C interrupt
		UINT32 INT_STATUS;
		gyro_readReg(gID, 0x19, (UINT32 *)&INT_STATUS);
	}
...
static ER gyro_writeRegOneByte(GYRO_SENSOR_ID gID, UINT32 uiAddr, UINT32 uiData)
{
	// write register
	{
		...
		spi_setTransferLen(gyroObj[gID].spiID, SPI_TRANSFER_LEN_1BYTE);
		spi_writeSingle(gyroObj[gID].spiID, (REG_WRITE) | (uiAddr & 0x7f));
		spi_writeSingle(gyroObj[gID].spiID, uiData);
		spi_setCSActive(gyroObj[gID].spiID, FALSE);
		spi_close(gyroObj[gID].spiID);
	}
	return E_OK;
...
static void gyro_syncEnd(GYRO_INNEROBJ *pGyroObj)
{
	...
	for (i = 0; i < *gyroQueueCount; i++) {
		...
		if (i % 2 == 0) {
			...
			pGyroObj->buf.type1.gyro[k][2] = (gyroData.vRecvWord[0] & 0xFF00) | ((gyroData.vRecvWord[0] >> 16) & 0xFF);
			//  y =  byte 3 | byte4
			pGyroObj->buf.type1.gyro[k][3] = ((gyroData.vRecvWord[0] >> 16) & 0xFF00) | (gyroData.vRecvWord[1] & 0xFF);
			//  z =  byte 5 | byte6
			pGyroObj->buf.type1.gyro[k][4] = (gyroData.vRecvWord[1] & 0xFF00) | ((gyroData.vRecvWord[1] >> 16) & 0xFF);
		} else {
			pGyroObj->buf.type1.accel[k][0] = gyroData.uiFrameID;
			pGyroObj->buf.type1.accel[k][1] = gyroData.uiDataID;

			//  x =  byte 1 | byte2 (byte 0 is address, we ignore it)
			pGyroObj->buf.type1.accel[k][2] = (gyroData.vRecvWord[0] & 0xFF00) | ((gyroData.vRecvWord[0] >> 16) & 0xFF);
			//  y =  byte 3 | byte4
			pGyroObj->buf.type1.accel[k][3] = ((gyroData.vRecvWord[0] >> 16) & 0xFF00) | (gyroData.vRecvWord[1] & 0xFF);
			//  z =  byte 5 | byte6
			pGyroObj->buf.type1.accel[k][4] = (gyroData.vRecvWord[1] & 0xFF00) | ((gyroData.vRecvWord[1] >> 16) & 0xFF);
...
#endif
	gyro_readReg(gID, REG_GYRO_XOUT_H, (UINT32 *)puiDatX);
	gyro_readReg(gID, REG_GYRO_YOUT_H, (UINT32 *)puiDatY);
	gyro_readReg(gID, REG_GYRO_ZOUT_H, (UINT32 *)puiDatZ);
	gyro_AdjustDataOffset(gID, puiDatX, puiDatY, puiDatZ);
	return E_OK;
}
...
ER gyro_getGsData(GYRO_SENSOR_ID gID, INT32 *puiDatX, INT32 *puiDatY, INT32 *puiDatZ)
{
	...
	if (gyroObj[gID].gyroOpMode == GYRO_OP_MODE_SINGLE_ACCESS) {
		gyro_readReg(gID, REG_ACCEL_XOUT_H, (UINT32 *)puiDatX);
		gyro_readReg(gID, REG_ACCEL_YOUT_H, (UINT32 *)puiDatY);
		gyro_readReg(gID, REG_ACCEL_ZOUT_H, (UINT32 *)puiDatZ);
	} else {
		UINT32 k;
		if (*gyroBufInIdx > 0) {
