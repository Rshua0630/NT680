...
extern void  sdiohost_enspreadspectrum(uint32_t id, bool ben);
extern bool  sdiohost_getrdy(struct mmc_nvt_host *host, uint32_t id);
extern void  sdiohost_enclock(struct mmc_nvt_host *host, uint32_t id);
extern void  sdiohost_disclock(struct mmc_nvt_host *host, uint32_t id);
...
extern bool  sdiohost_chkclock(struct mmc_nvt_host *host, uint32_t id);
extern bool  sdiohost_chkclkgating(uint32_t id);
extern void  sdiohost_setclkgating(uint32_t id, bool benable);
extern void  sdiohost_setclkgatinghigh(uint32_t id, bool bhigh);
extern int   sdiohost_setsrcclock(uint32_t id, uint32_t rate);
extern uint32_t sdiohost_getsrcclock(uint32_t id);
extern bool   sdiohost_chksrcclock(uint32_t id, uint32_t rate);
extern void   sdiohost_autopinmux(uint32_t id, bool bautopin);
extern int    sdiohost_enpinmux(uint32_t id, uint32_t uimaxbuswidth);
extern int    sdiohost_dispinmux(uint32_t id);
extern void   sdiohost_sinkbuspower(uint32_t id, bool bsink);
extern bool   sdiohost_ispinmuxopened(uint32_t id);
extern int     sdiohost_setcmddatapaddriving(uint32_t id, uint32_t cmddriving,
		uint32_t datadriving);
extern int    sdiohost_getpaddriving(uint32_t id, uint32_t *pdriving);
extern int    sdiohost_enint(uint32_t id);
extern int    sdiohost_disint(uint32_t id);

extern void   sdiohost_reset(struct mmc_nvt_host *host, uint32_t id);
extern void   sdiohost_resetasync(uint32_t id);
...
extern SDIO_HOST_VOLT_SWITCH_RESULT sdiohost_waitvoltageswitch(uint32_t id,
		uint32_t uitimeout);
extern REGVALUE sdiohost_getcmdresult(uint32_t id);
extern REGVALUE sdiohost_getdataresult(uint32_t id);
...
extern void     sdiohost_setclkcmdtype(struct mmc_nvt_host *host, uint32_t id,
		bool brisingsample);
/*extern void   sdiohost_setsampleedge(uint32_t id, bool bnegativesample);*/
/*extern void sdiohost_setsampleedgecmd(uint32_t id, bool bnegativesample);*/
/*extern void sdiohost_setinputdelay(uint32_t id, uint32_t uidlysetting);*/
/*extern void sdiohost_setoutputdelay(uint32_t id, uint32_t uidlysetting);*/
...
extern void sdiohost_dissdiointdetect(uint32_t id);
extern void sdiohost_ensdiointdetect(uint32_t id);
...
extern bool   sdiohost_chkvoltswitch(uint32_t id);

extern uint32_t sdiohost_waitio(uint32_t id);
...
extern int sdiohost_tuning_cmd(struct mmc_nvt_host *host, u32 opcode);
