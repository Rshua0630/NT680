/*
 * Novatek NA51000 PWM driver.
 *
 * Copyright (C) 2016 Novatek MicroElectronics Corp.
 *
 * Updated by Cliff Chen Aug 2016
 *
 * ----------------------------------------------------------------------------
 *
...
#define REG_INT_ENABLE		0x258
#define REG_INT_STS		0x268
...
static int nvt_pwm_enable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	err = clk_prepare_enable(pwm_clk[pwm->hwpwm]);
	if (err < 0) {
...
static void nvt_pwm_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	clk_disable_unprepare(pwm_clk[pwm->hwpwm]);
...
static int __init nvt_pwm_init_driver(void)
{
	...
	pwm_clk[12] = clk_get(NULL, "pwm_clk.12");
	pwm_clk[13] = clk_get(NULL, "pwm_clk.13");
	pwm_clk[14] = clk_get(NULL, "pwm_clk.14");
	pwm_clk[15] = clk_get(NULL, "pwm_clk.15");
	pwm_clk[16] = clk_get(NULL, "pwm_clk.16");
	pwm_clk[17] = clk_get(NULL, "pwm_clk.17");
	pwm_clk[18] = clk_get(NULL, "pwm_clk.18");
	pwm_clk[19] = clk_get(NULL, "pwm_clk.19");
