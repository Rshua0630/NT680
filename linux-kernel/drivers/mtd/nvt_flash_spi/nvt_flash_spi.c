...
{
	union NAND_DLL_PHASE_DLY_REG0 reg_dll_ctrl0;

	reg_dll_ctrl0.reg = NAND_GETREG(info, NAND_DLL_PHASE_DLY_REG0_OFS);

	reg_dll_ctrl0.bit.phy_data_dly_sel = 0x8;

	reg_dll_ctrl0.bit.phy_sw_reset = 1;

	NAND_SETREG(info, NAND_DLL_PHASE_DLY_REG0_OFS, reg_dll_ctrl0.reg);

	do {
		reg_dll_ctrl0.reg = NAND_GETREG(info, NAND_DLL_PHASE_DLY_REG0_OFS);
	} while(reg_dll_ctrl0.bit.phy_sw_reset == 1);
			...
			NAND_SPI_IO_ORDER io_order)
{
	...
	reg_io.bit.spi_cs_pol    = cs_pol;
	reg_io.bit.spi_bs_width  = bus_width;
	reg_io.bit.spi_io_order  = io_order;
