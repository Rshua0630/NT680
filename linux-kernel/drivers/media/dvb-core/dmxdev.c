...
#ifdef CONFIG_NVT_TK_DRIVER
		/*DVR record is implemented by hardware. */
		/*check the hardware DVR recorder.*/
...
static int dvb_dmxdev_feed_restart(struct dmxdev_filter *filter)
{
	...
	for (i = 0; i < dmxdev->filternum; i++)
		...
		if (dmxdev->filter[i].state >= DMXDEV_STATE_GO &&
			...
			if (dmxdev->demux->match_filter
				(dmxdev->filter[i].feed.sec,
...
static int dvb_demux_open(struct inode *inode, struct file *file)
{
	...
	dvb_ringbuffer_init(&dmxdevfilter->buffer, NULL,
		DEFAULT_DMX_BUFFER_SIZE);
	dmxdevfilter->type = DMXDEV_TYPE_NONE;
	dvb_dmxdev_filter_state_set(dmxdevfilter, DMXDEV_STATE_ALLOCATED);
...
#ifdef CONFIG_NVT_TK_DRIVER
	case DMX_GET_STATUS:
		dmxdev->demux->get_status(dmxdev->demux,
			(struct dmx_status *)parg);
		break;
#endif
	default:
		ret = -EINVAL;
			    ...
			    unsigned int cmd, void *parg)
{
	...
	/*		break;*/
	/*	}*/
	/*	ret = dmxdev->scr_dvr->set_data_type*/
	/*(dmxdev->scr_dvr, *(u32 *)parg);*/
	/*	break;*/
	case DVR_SET_SCR:
...
int dvb_dmxdev_init(struct dmxdev *dmxdev, struct dvb_adapter *dvb_adapter)
{
	...
	if (dmxdev->demux->capabilities & DMX_TS_HEADER_PARSER) {
		...
		dvb_ringbuffer_init(&dmxdev->dvr_hdr_buffer,
				NULL, DEFAULT_DMX_BUFFER_SIZE);
	}
#endif
