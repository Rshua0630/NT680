...
static void autoconfig_irq(struct uart_8250_port *up)
{
	...
	if (port->hw_flowctrl) {
		if (port->flags & UPF_FOURPORT) {
			serial_out(up, UART_MCR, UART_MCR_DTR | UART_MCR_AFC);
		} else {
		...
		if (port->flags & UPF_FOURPORT) {
			serial_out(up, UART_MCR, UART_MCR_DTR | UART_MCR_RTS);
		} else {
			serial_out(up, UART_MCR,
...
unsigned int serial8250_modem_status(struct uart_8250_port *up)
{
	...
	if (port->hw_flowctrl) {
		...
		if (status & (UART_MSR_ANY_DELTA | 0x10) &&
			up->ier & UART_IER_MSI && port->state != NULL) {
			if (status & UART_MSR_TERI)
				port->icount.rng++;
			if (status & UART_MSR_DDSR)
				port->icount.dsr++;
				     ...
				     unsigned int count)
{
	...
	if (up->canary && (up->canary != serial_port_in(port, UART_SCR))) {
		...
		if (port->hw_flowctrl)
			serial_port_out(port, UART_MCR,
				UART_MCR_DTR | UART_MCR_AFC);
		else
			serial_port_out(port, UART_MCR,
