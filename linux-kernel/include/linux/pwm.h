...
#if IS_ENABLED(CONFIG_PWM_NA51000)
static inline void pwm_set_cycle(struct pwm_device *pwm, unsigned int cycle)
{
	if (pwm)
		pwm->cycle = cycle;
}

static inline unsigned int pwm_get_cycle(struct pwm_device *pwm)
{
	return pwm ? pwm->cycle : 0;
}
#endif
